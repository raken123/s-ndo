"""Render the portrait gmfy 3.4.0 teasers. 1080x1920, 24fps."""
import os, subprocess, sys, time, wave
from multiprocessing import Pool
import numpy as np, imageio_ffmpeg

from gfx import W, H
from v340 import ADS
from admusic import music

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "outv340")
...
if __name__ == "__main__":
    t0 = time.time()
    # optional slug filter, so reworking one spot does not re-render all of them
    only = set(sys.argv[1:])
    ...
    for ai in want:
        ...
        with open(lst, "w") as f:
            for pp in parts:
                f.write("file '%s'\n" % pp)
        final = os.path.join(HERE, "gmfy_3.4.0_ad_%d_%s.mp4" % (ai + 1, slug))
