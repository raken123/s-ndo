...
def build_html():
    ...
    html = re.sub(
        ...
        'media-src * data: blob:; img-src \'self\' data: blob:;">',
        html, count=1)
    ...
    # inline the video ads as data URIs so the single file is self-contained
    import base64
    vids = {}
    for vid in VIDADS:
        vp = os.path.join(WWW, "ad", vid + ".mp4")
        if os.path.exists(vp):
            with open(vp, "rb") as f:
                vids[vid] = "data:video/mp4;base64," + base64.b64encode(f.read()).decode()
    blob = ("<script>window.GMFY_SRC=" + json.dumps(embed) +
            ";window.GMFY_VIDEO=" + json.dumps(vids) +
