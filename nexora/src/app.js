...
(function () {
  ...
  // Everything is generated by AI: settings from older versions that pointed at the removed offline generator move to Claude.
  if (S.settings.provider !== 'anthropic' && S.settings.provider !== 'openai') { S.settings.provider = 'anthropic'; store.set('settings', S.settings); }
  ...
  function providerLabel() {
    return ({ anthropic: 'Anthropic (Claude)', openai: 'Egen endpoint' }[S.settings.provider] || 'ingen') + (aiReady() ? '' : ' – inte inkopplad');
  }
  ...
  VIEWS.modeller = main => {
    ...
    main.append(h('section', { class: 'wrap' },
      ...
      h('div', { class: 'card', style: 'overflow-x:auto' }, h('table', { class: 't' },
        h('tr', null, h('th', null, 'Modell'), h('th', null, 'Anthropic'), h('th', null, 'Egen endpoint')), rows)),
  ...
  VIEWS.studio = main => {
    ...
    async function goGodot(prompt, features) {
      ...
      if (S.settings.provider !== 'anthropic') return settingsModal('Godot-läget använder Claude som agent (verktyg, skärmbilder och långa arbetspass). Välj Anthropic och lägg in din API-nyckel.');
  ...
  function settingsModal(note) {
    ...
    const prov = h('select', null, [['anthropic', 'Anthropic (Claude) – egen API-nyckel'], ['openai', 'Egen endpoint (OpenAI-kompatibel) – t.ex. din Colab-modell']].map(([v, l]) => h('option', { value: v, selected: s.provider === v }, l)));
