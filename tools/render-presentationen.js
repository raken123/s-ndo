/* Renderar reklam/reklam-presentationen.html till PNG-rutor: node tools/render-presentationen.js */
const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');
const dir = path.resolve(__dirname, '../frames4');
(async () => {
  fs.rmSync(dir, { recursive: true, force: true });
  fs.mkdirSync(dir, { recursive: true });
  const b = await chromium.launch({ args: ['--force-device-scale-factor=1', '--hide-scrollbars'] });
  const p = await b.newPage({ viewport: { width: 1920, height: 1080 } });
  await p.goto('file://' + path.resolve(__dirname, '../reklam/reklam-presentationen.html'));
